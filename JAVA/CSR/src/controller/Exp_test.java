	...
	public static void main(String[] args) throws IOException {
		...
		for (int j = 0; j < nbNodes; j++) {
			...
			for (int i = 0; i < amc.getMatrix().getRow(j).length; i++) 
			{
				...
				(amc.getClusterOfObjectI(j)==amc.getClusterOfObjectI(amc.getMatrix().getRow(j)[i].getLeft())? " qui se trouve dans le mÍme cluster" : " qui ne se trouve pas dans le mÍme cluster"));
		 ...
		 * == 0) { // Premi√®re ligne matrix_columns.add(new ArrayList<PairF
